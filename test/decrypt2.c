...
int do_evp_unseal(FILE *rsa_pkey_file, FILE *in_file, FILE *out_file)
{
    ...
    if (fread(iv, EVP_CIPHER_iv_length(EVP_aes_128_gcm()), 1, in_file) != 1)
    {
        perror("input file");
        retval = 4;
        goto out_free;
    ...
    if (!EVP_OpenInit(&ctx, EVP_aes_128_gcm(), ek, eklen, iv, pkey))
    {
    ...
    ret2 = EVP_OpenFinal(&ctx, buffer_out, &len_out);
	fprintf(stderr, "%d\n", ret2);
    if (ret2!=1)
    {
        perror( "EVP_SealFinal: failed.");
        retval = 3;
        goto out_free;
    }
